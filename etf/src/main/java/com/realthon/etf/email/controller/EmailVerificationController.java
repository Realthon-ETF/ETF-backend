...
import com.realthon.etf.email.dto.response.VerifyEmailCodeResponse;
...
public class EmailVerificationController {
    ...
    public ResponseEntity<SimpleMessageResponse> sendEmailCode(@RequestBody @Valid SendEmailCodeRequest request) {
        emailVerificationService.sendCodeIfUserExists(request.getUsername(), request.getPhoneNumber(), request.getEmail());
     ...
     */
    @PostMapping("/verify")
    public ResponseEntity<VerifyEmailCodeResponse> verifyEmailCode(@RequestBody @Valid VerifyEmailCodeRequest request) {
        String resetToken = emailVerificationService.verifyCodeAndIssueResetToken(request.getEmail(), request.getVerificationCode());
        return ResponseEntity.ok(new VerifyEmailCodeResponse(resetToken, emailVerificationService.getResetExpiresInSec()));
    }
}